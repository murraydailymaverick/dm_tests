describe('Visit urls on front end and ensure elements exist', () => {
	...
	it('tests the home page, nav, and footer on in Desktop 1280px', () => {
		...
		//dm-blocks
		cy.get('.wp-block-column .feed_item.three-col').should('have.length', 19)
		cy.get('.wp-block-column .top-reads-block ').should('have.length', 20)
		cy.get('.wp-block-column .top-reads-block .list-item').should('have.length', 87)
		...
		cy.get('.footer .col-sm-2 .footer-menu ul li a').should('have.length', 80)
	...
	it('tests the scorpio page on Desktop', () => {
		...
		//masthead
		cy.get('.masthead-intro').should('have.length', 1)
		cy.get('.categories-sublinks ul.list-inline li').should('have.length', 1)
